...
export interface IReview extends Document {
  ...
  markHelpful(userId: mongoose.Types.ObjectId): boolean;
  unmarkHelpful(userId: mongoose.Types.ObjectId): boolean;
}
...
export interface IProduct extends Document {
  ...
  category: 'Customize Your Prints' | 'Wear Your Conviction';
  designStyle?: 'Religious/Spiritual' | 'Motivational';
  description: string;
  shortDescription?: string;
  convictionMessage?: string;
  sizes: Array<{
    name: 'XS' | 'S' | 'M' | 'L' | 'XL' | '2XL' | '3XL';
    inStock: boolean;
    stockCount: number;
  }>;
  colors: Array<{
  ...
  canUserReview(userId: mongoose.Types.ObjectId): Promise<{ canReview: boolean; reason?: string; orderId?: mongoose.Types.ObjectId }>;
...
reviewSchema.methods.canBeEditedBy = function (userId: mongoose.Types.ObjectId) {
  return this.user.toString() === userId.toString();
};

reviewSchema.methods.markHelpful = function (userId: mongoose.Types.ObjectId) {
...
reviewSchema.methods.unmarkHelpful = function (userId: mongoose.Types.ObjectId) {
...
const productSchema = new Schema<IProduct>(
  {
    ...
    designStyle: {
      type: String,
      enum: ['Religious/Spiritual', 'Motivational'],
    },
    description: {
      type: String,
...
productSchema.virtual('verifiedReviewsPercentage').get(function (this: IProduct) {
...
// Methods
productSchema.methods.canUserReview = async function (userId: mongoose.Types.ObjectId) {
  const hasReviewed = this.reviews.some(
    (review: IReview) => review.user.toString() === userId.toString()
  );
  ...
  if (!hasPurchased)
    return { canReview: false, reason: 'Must purchase and receive product first' };
