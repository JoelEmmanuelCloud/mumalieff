...
import { addToWishlist, removeFromWishlist } from '@/lib/services/authService';
...
const ProductCard: React.FC<ProductCardProps> = React.memo(({ product, isWishlist = false }) => {
  const auth = useAuth();
  ...
  return (
    <article
      className="mobile-product-card bg-white rounded-lg shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:-translate-y-1 dark:bg-dark-card"
      onMouseEnter={() => {
        setIsHovered(true);
        preloadNextImage();
      }}
    ...
    >
      <div className="relative overflow-hidden">
        <Link href={`/product/${product._id}`} aria-label={`View ${product.name} details`}>
          <div className="mobile-product-image bg-gray-100 dark:bg-gray-800">
            <LazyImage
          ...
          {product.isSale && (
            <div className="mobile-badge bg-red-500 text-white" role="text" aria-label="On sale">
              Sale {Math.round(((product.price - (product.salePrice || 0)) / product.price) * 100)}% Off
            </div>
          )}
      ...
      <div className="mobile-card-content">
        <Link href={`/product/${product._id}`} className="block">
          <h3 className="mobile-product-title dark:text-white" itemProp="name">
            {product.name}
          </h3>
          ...
          <div className="flex items-center mb-2">
            ...
            >
              ...
              <meta itemProp="reviewCount" content={String(product.numReviews)} />
              {[...Array(5)].map((_, i) => (
                <svg
                  key={i}
                  xmlns="http://www.w3.org/2000/svg"
            ...
            <span className="mobile-review-count dark:text-gray-400" aria-label={`${product.numReviews} customer reviews`}>
          ...
          <div className="flex items-center justify-between">
            <div itemProp="offers" itemScope itemType="https://schema.org/Offer">
              ...
              {product.isSale ? (
                <div className="flex items-center space-x-2">
                  <span className="mobile-sale-price" itemProp="price" content={String(product.salePrice)}>
                ...
                <span className="mobile-price dark:text-white" itemProp="price" content={String(product.price)}>
                  ₦{product.price.toLocaleString()}
                </span>
              )}
            </div>
            ...
            {product.countInStock > 0 ? (
              <span className="mobile-stock-indicator text-green-600 dark:text-green-400" aria-label="Product in stock">
                In Stock
              </span>
            ) : (
              <span className="mobile-stock-indicator text-red-600 dark:text-red-400" aria-label="Product out of stock">
                Out of Stock
              </span>
            )}
          </div>
